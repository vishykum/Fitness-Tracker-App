...
const BodyFatChart: React.FC<BodyFatChartProps> = ({username, bodyfatData, darkMode}) => {
    ...
    const [max_bf, setMax_bf] = React.useState(0);
    const [min_bf, setMin_bf] = React.useState(0);
    ...
    React.useEffect(() => {
        ...
        setMin_bf(Math.min(...bodyfatData.map((item) => item.bodyfat)));
        setMax_bf(Math.max(...bodyfatData.map((item) => item.bodyfat)));
    }, [bodyfatData]);
        ...
        <>
            <div className={`w-full h-[50vh] mx-auto p-4 pl-0 ${(isFocussed) ? 'opacity-100' : 'opacity-80'} bg-yellow-400 dark:bg-orange-400`}
                ...
                >
                    ...
                    <YAxis domain={[min_bf, max_bf]} stroke="#212529"/>
